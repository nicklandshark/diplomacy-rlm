...
Seven AI agents play the board game Diplomacy autonomously -- negotiating alliances, coordinating attacks, betraying each other, and submitting orders -- powered by Recursive Language Models (RLMs).

Instead of stuffing the full game state into an ever-growing prompt (which degrades via "context rot"), each agent operates in a persistent Python REPL. Agents write code to query a read-only game view, maintain a markdown memory file on disk, and call sub-LM helpers for focused analysis. The LLM context stays small; the reasoning stays sharp across a multi-phase game.

## Architecture

Each of the 7 powers (Austria, England, France, Germany, Italy, Russia, Turkey) gets two agent tiers:
...
All agents run in parallel via `ThreadPoolExecutor`. Timed-out agents receive safe default orders (hold/disband/waive).
